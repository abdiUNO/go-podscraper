...
func CrawlGenre(e *colly.HTMLElement, categories map[string]bool) {
...
func NewPodScraper(options ...func(scraper *PodScraper)) *PodScraper {
	scr := &PodScraper{}
...
func WithInitUrl(initUrl string) func(*PodScraper) {
	return func(s *PodScraper) {
		s.InitUrl = initUrl
	}
}

func WithGenres(genres map[string]bool) func(*PodScraper) {
	return func(s *PodScraper) {
		s.Genres = genres
	}
}

func (s *PodScraper) Start() error {
	var initUrl = s.InitUrl

	// Instantiate default collector
	c := colly.NewCollector(
		colly.MaxDepth(4),
		colly.CacheDir("./cache/itunes_podcasts"),
	)

	detailCrawler := c.Clone()
	//Visit Genre
	c.OnHTML(genreQuery, func(e *colly.HTMLElement) { CrawlGenre(e, s.Genres) })
